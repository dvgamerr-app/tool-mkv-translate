...
const translateText = async (text) => {
  ...
  return await translations.join('\\N')
}

const assReader = async file => {
  const buff = await fs.readFile(file)
  // await fs.copyFile(file, `source_${path.basename(file).replace(path.extname(file), '')}${path.extname(file)}`)
  let pos = 0
  ...
  for await (let line of buff.toString().split('\n')) {
    ...
    if (/^Dialogue/ig.test(line)) {
      ...
      if (text) line = `Dialogue: ${style}${await translateText(text)}\n`
    }
